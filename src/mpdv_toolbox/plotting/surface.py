...
from mpdv_toolbox.analysis.surface_fit import surface
...
def plot_flyer_surface_3d(disp_df, popt, t_eval, positions_csv, reference_probe="probe_10", zrange = 20,
                           focus_scale=1.45, grid_size=300, elev=20, azim=45, ax=None):
    """3-D surface reconstruction of the flyer at time ``t_eval``."""

    time = disp_df["time"]
    t_eval_idx = np.argmin(np.abs(t_eval - time))
    p = tuple(popt.values[t_eval_idx,1:])

    # Generate meshgrid of flyer surface
    r = np.linspace(0,750e-6, 100)
    ...
    # Reshape and put into surface model
    ...
    # gather z data at correct time
    z_values = disp_df_ordered.values[t_eval_idx, :]

    # gather deflection data (w)
    w_values = z_values - disp_df[reference_probe].values[t_eval_idx]

    fig, ax = plt.subplots(figsize = (10,10), subplot_kw={"projection": "3d"})

    # Plot the fit data
    S = ax.plot_surface(xv*1e6, yv*1e6, zv*1e6, cmap="coolwarm",
                        linewidth=0, antialiased=False, alpha=0.25)

    # Plot the actual data
    P = ax.scatter3D(x_values*1e6, y_values*1e6, z_values*1e6, s=50, c=w_values*1e6, cmap="coolwarm",edgecolor='k')
    print(w_values)

    fig.colorbar(P, label="deflection w (microns)")

    ax.invert_zaxis()
    ax.set_xlabel("x (microns)")
    ax.set_ylabel("y (microns)")
    ax.set_zlabel("z (microns)")
    # min = z_values[~np.isnan(z_values)].mean()*1e6 - zrange/2
    # max = z_values[~np.isnan(z_values)].mean()*1e6 + zrange/2
    # ax.set_zlim([max, min])
    return fig, ax
